...
def l8mndwi(img):
    ndwi = img.normalizedDifference(['B2', 'B5']).rename('MNDWI');
    return img.addBands(ndwi);
...
def extentseries(reservoir):
    ...
    bandl8 = maskl8.select(["B1", "B2", "B3","B4","B5","B7"]);


    l8areas = bandl8.map(tareadate)
    l8areas = l8areas.filter(ee.Filter.gt('rarea', rarea.multiply(0.85)));
    
    ndwil8 = l8areas.map(l8ndwi);
...
for reservoirid in reservoirids:
    if os.path.exists('./data/sarea/L7/' + str(reservoirid) + '.txt') == False: 
        ...
        try:        
            ...
            for xxx in ppp:
                ...
                with open('./data/sarea/L7/' + str(reservoirid) + '.txt', 'w') as txt:
                    txt.write(series)
